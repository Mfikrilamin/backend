// package com.microservices.resourceserver.security;
...
// import
// com.microservices.resourceserver.security.WebSecurityConfig.SpringAddonsProperties.IssuerProperties;
...
// // Configure the app as resource-server with an authentication manager
// resolver
// // capable of handling multi-tenancy
// http.oauth2ResourceServer(
// resourceServer ->
// resourceServer.authenticationManagerResolver(authenticationManagerResolver));

// http.cors(cors ->
// cors.configurationSource(corsConfigurationSource(origins)));
...
// exchange.pathMatchers(permitAll).permitAll().anyExchange().authenticated());

// return http.build();
// }

// private UrlBasedCorsConfigurationSource corsConfigurationSource(String[]
// origins) {
...
// var buffer =
// dataBufferFactory.wrap(ex.getMessage().getBytes(Charset.defaultCharset()));
...
// final var issuerProperties = Stream.of(issuers).filter(iss ->
// issuerUri.equals(iss.getUri())).toList();
// if (issuerProperties.size() == 0) {
// throw new MisconfigurationException(
// "Missing authorities mapping properties for
// %s".formatted(issuerUri.toString()));
// }
// if (issuerProperties.size() > 1) {
// throw new MisconfigurationException(
// "Too many authorities mapping properties for
// %s".formatted(issuerUri.toString()));
// }
...
// static class JwtGrantedAuthoritiesConverter implements Converter<Jwt,
...
// final var authorities = new
// JwtGrantedAuthoritiesConverter(issuerProperties).convert(jwt);
// final String username = JsonPath.read(jwt.getClaims(),
// issuerProperties.getUsernameJsonPath());
...
// .map(IssuerProperties::getUri)
// .map(URL::toString).collect(Collectors.toMap(issuer -> issuer,
// issuer -> Mono.just(authenticationManager(issuer,
...
// JwtReactiveAuthenticationManager authenticationManager(String issuer,
// SpringAddonsJwtAuthenticationConverter authenticationConverter) {
